...
本地需要 Go 1.25+ 和 Docker Compose。MySQL、NapCat 可以直接用仓库里的 `docker-compose.yaml` 启动。
...
compose 只启动外部依赖，不运行 Go bot。bot 保持本地单独运行，方便调试和后续部署。
...
MySQL 首次初始化时会自动执行该 SQL。若 `mysql_data` volume 已存在，初始化 SQL 不会重复执行。
...
| `scripts` | 代码生成和工具安装脚本 |
| `data/cache` | WPS 知识表下载缓存 |
| `Dockerfile` | bot 容器镜像构建文件 |
| `docker-compose.yaml` | 本地 MySQL、NapCat 和 quote 依赖 |
