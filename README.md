...
`go generate` 会通过 `scripts/gormgen.sh` 调用固定版本 `gentool`，读取当前 MySQL 表结构并输出到 `internal/storage/query`。`gentool` 不会自动执行 schema SQL；如果 `mysql_data` volume 已存在且 schema 有变化，需要先按上面的说明重建空库或手动执行 SQL。
