...
`make gormgen` 会设置 `JXH_GORMGEN_DSN` 并通过 `go generate ./internal/storage` 调用 `scripts/gormgen.sh`，读取当前 MySQL 表结构并输出到 `internal/storage/query`。`gentool` 不会自动执行 schema SQL；如果 `mysql_data` volume 已存在且 schema 有变化，需要先按上面的说明重建空库或手动执行 SQL。
...
| --- | --- |
| `cmd/bot` | bot 启动入口 |
| `scripts/gormgen.sh` | 通过 gentool 根据 MySQL schema 生成 GORM query/model |
