...
CREATE TABLE IF NOT EXISTS `knowledge_entries` (
  ...
  `content_hash` varchar(64) NOT NULL COMMENT 'content sha256，用于判断向量是否需要重建',
  `vector_status` varchar(16) NOT NULL DEFAULT 'pending' COMMENT '向量同步状态：pending/ready/failed',
  `vector_content_hash` varchar(64) DEFAULT NULL COMMENT '已同步向量对应的 content_hash',
  `vector_synced_at` datetime(3) DEFAULT NULL COMMENT '向量同步时间',
