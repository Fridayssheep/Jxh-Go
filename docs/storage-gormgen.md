...
当前表清单只有 `knowledge_trigger_logs`、`scheduled_jobs` 和 `group_join_requests`。新增 MySQL 表时先更新 `deploy/mysql/init/001_schema.sql`，再同步 `scripts/gormgen.sh` 的 `-tables` 参数。
